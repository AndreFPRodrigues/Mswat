 ...
 * 
 * Feed all the touch positions via store(), when it returns true a type of touch has finished.
 * Use identifyTouch() to get the type of touch iden
...
public class TouchPatternRecognizer {
		...
		if (touches.size() < 5){
			...
			for (int i = 1; i < touches.size(); i++) {
				distance = Math.sqrt((Math.pow(
						(touches.get(i).getY() - touches.get(
								i - 1).getY()), 2) + Math
						.pow((touches.get(i).getX() - touches
								.get(i - 1).getX()), 2)));
				if (distance > 50){
	 ...
	 * @return 
	 * 	(-1) -> no touch identified yet
	 * 	 (0) -> touch identified
	 *   (1) -> slide identified
	 *   (2) -> LongPress identified
	 */
